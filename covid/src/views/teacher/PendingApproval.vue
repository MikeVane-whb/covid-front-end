<template>
  <div>
    ...
    <el-table :data="leaveRecord" border
      ...
      <el-table-column prop="gradeClass" label="班级"  align="center" sortable/>
      ...
      <el-table-column label="操作" align="center" min-width="150">
        <template v-slot="scope">
          ...
          <el-button type="success" @click="agree(scope.row)">同意 <i class="el-icon-check"></i></el-button>
          <el-button type="danger" @click="disagree(scope.row)">拒绝 <i class="el-icon-close"></i></el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {StudentLeaveInfo} from "@/data/StudentDataDefine";

const CurrentURL = "/teacher/pendApproval"
export default {
  ...
  methods:{
    load(){
      request.get(CurrentURL + "/getPendApproval", {
        params: {
    ...
    agree(value){
      this.$confirm('确定要同意'+value.username+'的请假申请', '确认信息', {
        distinguishCancelAndClose: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        value.status = 1
        request.post(CurrentURL + '/updateStatus', value).then(res => {
          if (res.code === this.getStatusCode('SUCCESS')) {
    ...
    disagree(value){
      this.$confirm('确定要拒绝'+value.username+'的请假申请', '确认信息', {
        distinguishCancelAndClose: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
